...
const deleteFileIfExists = (fileName) => {
  if (!fileName) return;
  const fullPath = path.join(process.cwd(), "public/uploads/new", fileName);
...
const getNew = async (req, res) => {
  try {
    ...
    const result = await pool.request().query("SELECT * FROM new");
    res.json(result.recordset);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "new error" });
  }
};
...
const getNewById = async (req, res) => {
  ...
  try {
    ...
    if (result.recordset.length === 0) return res.status(404).json({ error: "New not found" });
...
const updateNew = async (req, res) => {
  ...
  try {
    ...
    const old = await pool.request()
      .input("id", sql.Int, id)
      .query("SELECT image_1, image_2, image_3 FROM new WHERE id=@id");
    ...
    res.json({ message: "New updated successfully" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "New error" });
  }
...
const deleteNew = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "New deleted successfully" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "New error" });
  }
...
module.exports = { getNew, getNew, createNew, updateNew, deleteNew,getNewById };
