...
const getVisits = async (req, res) => {
  try {
    ...
    const result = await pool.request().query("SELECT * FROM visit");
...
const getVisitById = async (req, res) => {
  ...
  try {
    ...
    if (result.recordset.length === 0) return res.status(404).json({ error: "Visit not found" });
...
const updateVisit = async (req, res) => {
  ...
  try {
    ...
    const old = await pool.request()
      .input("id", sql.Int, id)
      .query("SELECT image_1, image_2, image_3 FROM visit WHERE id=@id");

    if (old.recordset.length === 0) return res.status(404).json({ error: "Visit not found" });
